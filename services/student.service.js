...
export const getStudentById = tryToCatch(async (id) => {
...
export const updateStudent = tryToCatch(
  async (id, userId, userRole, updateData) => {
...
export const getStudentCVs = tryToCatch(async (id) => {
  const student = await Student.findById(id).populate("user", "-password");
  if (!student) {
    throw new Error("Student not found");
  }
